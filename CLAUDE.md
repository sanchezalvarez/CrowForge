...
Use semver format (e.g., `0.5.2` not `0.4`) for all version strings. Current version: **0.5.2** (set in `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, displayed in `App.tsx` sidebar and `SettingsPage.tsx` About). Always validate version format before committing.
